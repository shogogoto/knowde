...
from knowde.tmp.definition.term import TermUtil
...
def test_add_mark() -> None:
    ...
    s1 = add_description(Description(value=f"xx{t1}xx")).to_model()
    ...
    s3 = add_description(Description(value=f"xx{t1}xx{t2}xx")).to_model()
...
def test_remove_mark() -> None:
    ...
    s = add_description(Description(value=f"xx{t1}x{t2}x{t3}xx")).to_model()
...
def test_change_mark() -> None:
    ...
    t1 = TermUtil.create(value="t1").to_model()
    t2 = TermUtil.create(value="t2").to_model()
    s = add_description(Description(value=f"xx{t1}x{t2}xx")).to_model()
    assert find_marked_terms(s.valid_uid) == [t1, t2]

    t3 = TermUtil.create(value="t3").to_model()
    s2 = remark_sentence(s.valid_uid, Description(value=f"xx{t3}xx")).to_model()
