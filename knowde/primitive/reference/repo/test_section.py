...
from knowde.primitive.reference.repo.chapter import add_book_chapter, remove_chapter
from knowde.primitive.reference.repo.label import BookUtil, SectionUtil
...
def test_swap_section() -> None:  # noqa: D103
...
def test_remove_section_and_reorder() -> None:  # noqa: D103
    book = BookUtil.create(title="book")
    b = book.to_model()
    c = add_book_chapter(b.valid_uid, h("h1"))
    s1 = add_section(c.valid_uid, h("s1"))
...
def test_remove_chapter_with_sections() -> None:  # noqa: D103
    book = BookUtil.create(title="book")
    b = book.to_model()
