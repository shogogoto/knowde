...
def find_adjacent(concept_uid: UUID) -> AdjacentConcept:
    ...
    lb = util_concept.find_by_id(concept_uid)
...
def connect(from_uid: UUID, to_uid: UUID) -> None:
    ...
    cfrom = util_concept.find_by_id(from_uid).label
    cto = util_concept.find_by_id(to_uid).label
...
def disconnect_all(concept_uid: UUID) -> None:
    lc = util_concept.find_by_id(concept_uid).label
