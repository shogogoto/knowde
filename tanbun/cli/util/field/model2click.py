...
from tanbun.cli.util.to_click import (
    ClickDecorator,
    ClickParam,
    to_clicktype,
)
from tanbun.cli.util.typeutil import (
    extract_type,
    is_nested,
    is_option,
    is_optional,
)
from tanbun.cli.util.typeutil.check import is_generic_alias
...
def field2option_attrs(info: FieldInfo) -> OptionAttrs:  # noqa: D103
    nargs = 1
...
def field2argument_attrs(info: FieldInfo) -> ArgumentAttrs:  # noqa: D103
    nargs = 1
    if is_generic_alias(info.annotation):
        nargs = -1
    return {"nargs": nargs}
